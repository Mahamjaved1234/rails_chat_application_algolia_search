class SubcategoriesController < ApplicationController
  ...
  def index
    red = Redis.new
    @subcategories = Subcategory.where(category_id: red.get("category_id").to_i)
  end
  ...
  private
    ...
    def set_subcategory
      ...
      red.set("subcategory_id","#{@subcategory.category_id}")
      puts"========= REDIS SAVED SUB-CATEGORY ID #{red.get("subcategory_id")} ========="
      @product = Product.where(subcategory_id: red.get("subcategory_id"))
      id = Category.where(id:red.get("category_id").to_i)
    end
